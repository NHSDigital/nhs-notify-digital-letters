/**
 * Tests for discover-schema-dependencies.js
 *
...
const SCRIPT_PATH = path.resolve(__dirname, '../discover-schema-dependencies.js');
...
describe('discover-schema-dependencies CLI', () => {
  ...
  describe('CLI argument validation', () => {
    ...
    it('should exit with error when only one argument provided', () => {
      expect(() => {
        execSync(`node ${SCRIPT_PATH} /some/path`, {
          encoding: 'utf-8',
          stdio: 'pipe',
        });
      }).toThrow();
    ...
    it('should exit with error when root schema file does not exist', () => {
      ...
      expect(() => {
        execSync(`node ${SCRIPT_PATH} ${nonExistentPath} ${outputDir}`, {
          encoding: 'utf-8',
          stdio: 'pipe',
        });
      }).toThrow();
...
`;
      ...
      const result = execSync(`node ${SCRIPT_PATH} ${schemaPath} ${outputDir}`, {
        encoding: 'utf-8',
      });
...
`;
      ...
      const result = execSync(`node ${SCRIPT_PATH} ${schemaAPath} ${outputDir}`, {
        encoding: 'utf-8',
...
`, 'utf-8');
      ...
      const result = execSync(`node ${SCRIPT_PATH} ${schema1Path} ${outputDir}`, {
        encoding: 'utf-8',
      });
...
`, 'utf-8');
      ...
      const result = execSync(`node ${SCRIPT_PATH} ${eventPath} ${outputDir}`, {
        encoding: 'utf-8',
      });
