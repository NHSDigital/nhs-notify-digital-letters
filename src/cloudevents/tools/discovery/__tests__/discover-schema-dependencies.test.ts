...
describe('discover-schema-dependencies CLI', () => {
  ...
  describe('CLI argument validation', () => {
    it('should exit with error when no arguments provided', () => {
      expect(() => {
        execFileSync('npx', ['tsx', SCRIPT_PATH], { encoding: 'utf-8', stdio: 'pipe' });
      }).toThrow();
    });
    ...
    it('should exit with error when root schema file does not exist', () => {
      ...
      expect(() => {
        execFileSync('npx', ['tsx', SCRIPT_PATH, nonExistentPath, outputDir], {
          encoding: 'utf-8',
          stdio: 'pipe',
        });
      }).toThrow();
...
`;
      ...
      const result = execFileSync('npx', ['tsx', SCRIPT_PATH, schemaPath, outputDir], {
        encoding: 'utf-8',
      });
...
`;
      ...
      const result = execFileSync('npx', ['tsx', SCRIPT_PATH, schemaAPath, outputDir], {
        encoding: 'utf-8',
...
`, 'utf-8');
      ...
      const result = execFileSync('npx', ['tsx', SCRIPT_PATH, schema1Path, outputDir], {
        encoding: 'utf-8',
      });
...
`, 'utf-8');
      ...
      const result = execFileSync('npx', ['tsx', SCRIPT_PATH, eventPath, outputDir], {
        encoding: 'utf-8',
      });
