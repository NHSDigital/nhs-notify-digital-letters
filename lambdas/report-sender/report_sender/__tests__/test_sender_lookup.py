...
class TestSenderLookup:
    ...
    def test_get_mailbox_from_sender_success(self, mock_ssm, mock_config):
    ...
    def test_get_mailbox_from_sender_not_found(self, mock_ssm, mock_config):
        ...
        with pytest.raises(Exception) as exc_info:
            lookup.get_mailbox_from_sender(sender_id)
    ...
    def test_get_mailbox_from_sender_missing_value_field(self, mock_ssm, mock_config):
        ...
        with pytest.raises(Exception) as exc_info:
            lookup.get_mailbox_from_sender(test_sender_id)

        assert "missing a 'Value' field" in str(exc_info.value)

    def test_get_mailbox_from_sender_invalid_json(self, mock_ssm, mock_config):
        ...
        assert "Failed to parse mailbox ID" in str(exc_info.value)
