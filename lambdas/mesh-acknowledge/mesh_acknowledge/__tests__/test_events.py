...
def create_invalid_sqs_record(event_id: str) -> Dict[str, str | int]:
    """Create a valid SQS record with MESHInboxMessageDownloaded event"""
    return {
        'body': json.dumps({
            'detail': {
...
class TestPublishAcknowledgedEvent:
    ...
    @patch('mesh_acknowledge.events.uuid4')
    @patch('mesh_acknowledge.events.datetime')
    def test_publish_invalid_event_raises_error(
        self,
        mock_datetime,
        mock_uuid,
        mock_logger,
        ...
        """Test that invalid event construction raises error"""
        # Using an invalid UUID will cause validation of the new
        # MESHInboxMessageAcknowledged event to fail
        mock_uuid.return_value = 'uuid-error'
