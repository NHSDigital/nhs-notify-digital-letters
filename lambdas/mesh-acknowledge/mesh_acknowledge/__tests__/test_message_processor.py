...
def create_message_processor(
        ...
        mock_sender_lookup, mock_logger):
...
class TestMessageProcessorProcessMessage:
    ...
    @patch('mesh_acknowledge.message_processor.publish_acknowledged_event')
    def test_process_message_skips_non_sqs_records(
        self,
        mock_publish,
        message_processor,
        mock_acknowledger,
    ):
        """Test that non-SQS records are skipped"""
        message = {
            'Records': [
                {
                    'messageId': 'sns-msg-1',
    ...
    @patch('mesh_acknowledge.message_processor.publish_acknowledged_event')
    @patch('mesh_acknowledge.message_processor.parse_downloaded_event')
    def test_process_message_publish_error_returns_failure(
        self,
        mock_parse,
        mock_publish,
        message_processor,
        ...
        """Test that publish errors are caught and returned as batch failures"""
        mock_parse.return_value = downloaded_event
        mock_sender_lookup.get_mailbox_id.return_value = "MAILBOX001"
        mock_acknowledger.acknowledge_message.return_value = "ACK123"
