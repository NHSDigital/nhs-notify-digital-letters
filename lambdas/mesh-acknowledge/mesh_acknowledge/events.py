...
def parse_downloaded_event(sqs_record, logger) -> MESHInboxMessageDownloaded:
    ...
    try:
        ...
        logger.error(
            "Error parsing SQS message body as JSON",
...
def publish_acknowledged_event(
    ...
    try:
        acknowledged_event = MESHInboxMessageAcknowledged(**{
        ...
        failed = event_publisher.send_events([acknowledged_event])
        ...
        logger.error(
            "Invalid MESHInboxMessageAcknowledged event", error=str(e))
        raise
