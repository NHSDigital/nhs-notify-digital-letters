module "file_scanner_move_scanned_files" {
  source = "https://github.com/NHSDigital/nhs-notify-shared-modules/releases/download/v2.0.29/terraform-lambda.zip"

  function_name = "file-scanner-move-scanned-files"
  ...
  iam_policy_document = {
    body = data.aws_iam_policy_document.file_scanner_move_scanned_files.json
  }

  function_s3_bucket      = local.acct.s3_buckets["lambda_function_artefacts"]["id"]
  function_code_base_path = local.aws_lambda_functions_dir_path
  function_code_dir       = "file-scanner-move-scanned-files-lambda/dist"
...
data "aws_iam_policy_document" "file_scanner_move_scanned_files_lambda" {
  ...
  statement {
    ...
    resources = [
      module.sqs_file_scanner_move_scanned_files.sqs_queue_arn,
    ]
  }
